...
import {
  ...
  Button, Separator, ScrollArea, Badge, toast,
} from "@rootcx/ui";
import { IconPlus, IconEdit, IconTrash, IconBuilding, IconArrowLeft, IconNotes, IconWorld, IconPhone, IconMapPin } from "@tabler/icons-react";
...
const FORM_FIELDS = [
  ...
  { name: "industry", label: "Industry",     type: "select" as const, options: Object.keys(INDUSTRY_COLORS).map(i => ({ label: i, value: i })) },
...
export default function CompaniesView() {
  ...
  const openEdit = (company: Company) => { setEditTarget(company); setFormOpen(true); };
  ...
  const columns: ColumnDef<Company, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => row.original.industry
        ? <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium", INDUSTRY_COLORS[row.original.industry] ?? "bg-gray-100 text-gray-700")}>{row.original.industry}</span>
        : "—",
    },
    {
      accessorKey: "website",
    ...
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        actions={
          <Button onClick={() => { setEditTarget(null); setFormOpen(true); }}>
            <IconPlus className="h-4 w-4 mr-1.5" /> Add Company
          </Button>
        }
      />
      ...
      <DataTable
        ...
        bulkActions={[{
          ...
          onClick: async rows => { await Promise.all(rows.map(r => remove(r.id))); toast.success(`${rows.length} companies deleted`); },
          destructive: true,
        }]}
        emptyState={
          <EmptyState
            ...
            action={<Button onClick={() => { setEditTarget(null); setFormOpen(true); }}><IconPlus className="h-4 w-4 mr-1.5" /> Add Company</Button>}
          />
        }
      />
