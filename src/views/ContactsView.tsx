...
export default function ContactsView({ onSelectContact }: { onSelectContact: (id: string) => void }) {
  ...
  const columns: ColumnDef<Contact, unknown>[] = [
    ...
    { accessorKey: "status",     header: "Status",  cell: ({ row: { original: r } }) => r.status ? <StatusBadge status={STATUS_MAP[r.status] ?? "default"} label={r.status} /> : "—" },
  ];
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <ConfirmDialog open={!!deleteTarget} onOpenChange={o => !o && setDeleteTarget(null)}
        title="Delete Contact" description={`Are you sure you want to delete ${deleteTarget?.first_name} ${deleteTarget?.last_name}? This cannot be undone.`}
